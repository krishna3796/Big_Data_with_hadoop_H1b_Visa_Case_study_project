load_data = load '/user/hive/warehouse/h1b.db/h1b_final' using PigStorage('\t') as (s_no:int,case_status:chararray,employer_name:chararray,soc_name:chararray,job_title:chararray,full_time_position:chararray,prevailing_wage:long,year:chararray,worksite:chararray,longitute:double,latitute:double);

groupby_emp = group load_data by employer_name;
...
group_certified_filter_data = group filter_certified by employer_name;
...
group_certified_withdrawn_filter_data = group filter_certified_withdrawn by employer_name;
